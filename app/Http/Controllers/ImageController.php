...
class ImageController extends Controller
{
  ...
  public function verifyImage($image_array, $category_id, $is_featured, $is_catalog)
  {
    ...
    if (!($extension == 'png' || $extension == 'jpeg' || $extension == 'jpg'))
      $response = Response::json(array('code' => 201, 'message' => 'Please select PNG or JPEG/JPG file.', 'cause' => '', 'data' => json_decode("{}")));
  ...
  public function verifyImagesArray($images_array, $image_type, $category_id, $is_featured, $is_catalog)
  {
    ...
    if (sizeof($files_array) > 0) {
      ...
      return $response = Response::json(array('code' => 436, 'message' => 'File is not verified by size or format. Please check error list.', 'cause' => '', 'data' => $result));
    } else {
      return $response = '';
    }
  }
  ...
  public function verifyVideo($video_array)
  {
    ...
    if (!($video_type == 'video/x-ms-asf' || $video_type == 'video/mp4' || $video_type == 'video/webm' || $video_type == 'video/quicktime' || $video_type == 'application/octet-stream')) {
      ...
      return $response = Response::json(array('code' => 201, 'message' => 'File size is greater then 5MB.', 'cause' => '', 'data' => json_decode("{}")));
    } else
      $response = '';
    return $response;
  }
...
//    Log::info('$MAXIMUM_FILESIZE : ', ['MAXIMUM_FILESIZE' => $MAXIMUM_FILESIZE]);
    ...
    if (!($audio_type == 'audio/mpeg' || $audio_type == 'application/octet-stream')) {
      return $response = Response::json(array('code' => 201, 'message' => 'Please select 3gp or mp3 audio file.', 'cause' => '', 'data' => json_decode("{}")));
    } elseif ($audio_size > $MAXIMUM_FILESIZE) {
      return $response = Response::json(array('code' => 201, 'message' => 'File size is greater then '.$validations.'KB.', 'cause' => '', 'data' => json_decode("{}")));
    } else
      $response = '';
    return $response;
  }
  ...
  public function verifyStlFile($file_array)
  {
    ...
    if (!($file_type == 'application/octet-stream'))
      $response = Response::json(array('code' => 201, 'message' => 'Please select stl file.', 'cause' => '', 'data' => json_decode("{}")));
...
//      return $new_file_name;
      return array('code' => 200, 'msg' => $new_file_name);
    } catch (Exception $e) {
...
//      Input::file('file')->move($original_path, $file_name);
      ...
      if ($width_orig < 200 or $height_orig < 200) {
        ...
        $file_info = array('height' => $height, 'width' => $width, 'file_name' => $file_name_without_ext . '.webp');
      } else {
        ...
        $file_info = array('height' => $height_orig, 'width' => $width_orig, 'file_name' => $file_name_without_ext . '.webp');
      }
      ...
      return array('code' => 200, 'msg' => $file_info);
//      return $file_info;
      ...
      foreach ($image_array as $key) {
        ...
        if (Config::get('constant.STORAGE') === 'S3_BUCKET') {
          ...
          if ($disk->exists($value)) {
            $exist_files_array[] = array('url' => Config::get('constant.RESOURCE_IMAGES_DIRECTORY_OF_DIGITAL_OCEAN') . $image, 'name' => $image);
          }
        } else {
    ...
    try {
      ...
      if (Config::get('constant.STORAGE') === 'S3_BUCKET') {
        ...
        if ($disk->exists($value)) {
          ...
          $exist_files_array[] = array('url' => Config::get('constant.3D_OBJECTS_DIRECTORY_OF_DIGITAL_OCEAN') . $file, 'name' => $file);
        }
      } else {
...
//      Log::error("generatePreviewImage : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      return $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'generate preview image.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    }
  }
...
//        $response = 1;
        $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'upload this video file.', 'cause' => '', 'data' => json_decode("{}")));
      }
    } catch (Exception $e) {
...
//      Log::error("deleteVideo : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      return Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'delete video.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    }
  }
      ...
      foreach ($file_array as $key) {
        ...
        if (Config::get('constant.STORAGE') === 'S3_BUCKET') {
          ...
          if ($disk->exists($value)) {
            ...
            $exist_files_array[] = array('url' => $file_url, 'name' => $file);
          }
        } else {
  ...
  public function checkIsVideoExist($video_name)
  {
    try {
      ...
      if (sizeof($exist_files_array) > 0) {
        ...
        return $response = Response::json(array('code' => 434, 'message' => 'File does not exist. File name : ' . $video_name, 'cause' => '', 'data' => json_decode("{}")));
  ...
  public function verifySampleImageForVideo($image_array)
  {
    ...
    */
    ...
    if (!($image_type == 'image/png' || $image_type == 'image/jpeg'))
      $response = Response::json(array('code' => 201, 'message' => 'Please select PNG or JPEG file', 'cause' => '', 'data' => json_decode("{}")));
    elseif ($image_size > $MAXIMUM_FILESIZE)
      $response = Response::json(array('code' => 201, 'message' => "Sample image is greater then $MAX_SIZE_IN_KB KB", 'cause' => '', 'data' => json_decode("{}")));
    else
      $response = '';
    return $response;
...
//    Log::info("Image Size",[$image_size]);
    ...
    if (!($image_type == 'image/png' || $image_type == 'image/jpeg'))
      $response = Response::json(array('code' => 201, 'message' => 'Please select PNG or JPEG file', 'cause' => '', 'data' => json_decode("{}")));
    elseif ($image_size > $MAXIMUM_FILESIZE)
      $response = Response::json(array('code' => 201, 'message' => ucfirst($file_name) . ' size is greater then 150KB, Please adjust size by zooming out with zoom adjustment slider.', 'cause' => '', 'data' => json_decode("{}")));
    else
      $response = '';
    return $response;
  }
...
//      Log::error("deleteVideoFromJob : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      return Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'delete video from job.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    }
  }
...
//    Log::info("Image Size",[$image_size]);
    ...
    if (!($image_type == 'image/png' || $image_type == 'image/jpeg'))
      $response = Response::json(array('code' => 201, 'message' => 'Please select PNG or JPEG file.', 'cause' => '', 'data' => json_decode("{}")));
    elseif ($image_size > $MAXIMUM_FILESIZE)
      $response = Response::json(array('code' => 201, 'message' => 'File size is greater then 2MB.', 'cause' => '', 'data' => json_decode("{}")));
    else
      $response = '';
    return $response;
  }
...
//        }
        ...
        $fail_reason = json_encode(array('download_id' => $download_id, 'audio_trim_output' => $output));
...
//      Log::error("trimAudioByJob : ", ['Exception' => $e->getMessage(), '\nTraceAsString' => $e->getTraceAsString()]);
      $fail_reason = json_encode(array('Exception' => $e->getMessage()));
      (new userController())->addVideoGenerateHistory($fail_reason,NULL,$download_id,NULL,NULL,NULL,2);
      $response = '';
    }
    return $response;
...
//    Log::info("verifyFontJsonFile : ", ['type' => $file_type]);
    ...
    if (!($file_type == 'application/json' || $file_type == 'text/plain' || $file_type == 'application/octet-stream'))
      $response = Response::json(array('code' => 201, 'message' => 'Please select json file.', 'cause' => '', 'data' => json_decode("{}")));
    else
      $response = '';
    return $response;
  }
  ...
  public function checkIsAudioExist($audio_name)
  {
    try {
      ...
      if (sizeof($exist_files_array) > 0) {
        ...
        return $response = Response::json(array('code' => 434, 'message' => 'File does not exist. File name : ' . $audio_name, 'cause' => '', 'data' => json_decode("{}")));
  ...
  public function checkIsResourceImageExist($image_name)
  {
    try {
      ...
      if (sizeof($exist_files_array) > 0) {
        ...
        return $response = Response::json(array('code' => 434, 'message' => 'File does not exist. File name : ' . $image_name, 'cause' => '', 'data' => json_decode("{}")));
