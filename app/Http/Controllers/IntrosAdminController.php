  ...
  public function addIntrosVideoByAdmin(Request $request_body)
  {
    try {
      ...
      if (!$request_body->hasFile('sample_video'))
        return Response::json(array('code' => 201, 'message' => 'Required field sample_video is missing or empty.', 'cause' => '', 'data' => json_decode("{}")));
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('catalog_id', 'is_featured', 'is_free', 'search_category', 'template_name'), $request)) != '')
        return $response;
...
//                (new ImageController())->unlinkFileFromLocalStorage($resource_image_name, Config::get('constant.RESOURCE_IMAGES_DIRECTORY'));
      ...
      if (empty($json_file_array)) {
        return Response::json(array('code' => 201, 'message' => 'Required file json is missing in zip file.', 'cause' => '', 'data' => json_decode("{}")));
      }
      ...
      if ($result['result_status'] == 0) {
        ...
        $response = Response::json(array('code' => 200, 'message' => 'Template added successfully.', 'cause' => '', 'data' => ['preview_id' => $preview_id, 'content_id' => $content_id,'template_detail' => $template_details ]));
      }
    } catch (Exception $e) {
      (new ImageController())->logs("addIntrosVideoByAdmin",$e);
//      Log::error("addIntrosVideoByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . ' add intros video by admin.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      /** Delete json file from temp folder */
...
//        return $response;
      if (($response = (new VerificationController())->validateRequiredParameter(array('content_id', 'is_featured', 'is_free','search_category', 'template_name'), $request)) != '')
        return $response;
...
//
      ...
      if (count($content_detail) <= 0) {
        return Response::json(array('code' => 201, 'message' => 'Template does not exist,Invalid content_id', 'cause' => '', 'data' => json_decode("{}")));
      }
...
//      DB::rollBack();
  ...
  public function generateRowVideo(Request $request_body)
  {
    try {
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Row videos successfully.', 'cause' => '', 'data' => ['total_record' => $total_row, 'is_next_page' => $is_next_page,'total_generated_video' =>$preview_count,"fail_ids" => $fail_id_array]));

    } catch(Exception $e) {
      (new ImageController())->logs("generateRowVideo",$e);
//      Log::error("generateRowVideo : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . ' compresse row video.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
    }
    return $response;
  }
