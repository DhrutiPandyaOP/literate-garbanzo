...
class ImageExportController extends Controller
{
    ...
    public function generateDesign(Request $request_body)
    {
        try {
            ...
            if (Config::get('constant.IS_RENDER_SERVER_WORKING') == 0) {
                $response = Response::json(['code' => 205, 'message' => Config::get('constant.EXCEPTION_ERROR').'export '.$size.'x sized file. Please Try Again.', 'cause' => '', 'data' => json_decode('{}')]);
            ...
            if ($queue_record[0]->total >= $queue_limit) {
                return Response::json(['code' => 201, 'message' => 'You can\'t add more than '.$queue_limit.' designs for download at a time. Please try after some time.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            $image_list = ['my_design_id' => $uuid, 'folder_id' => $folder_id, 'user_id' => $user_uuid, 'sub_category_id' => $sub_category_uuid, 'sample_image' => Config::get('constant.MY_DESIGN_IMAGES_DIRECTORY_OF_DIGITAL_OCEAN').$card_image, 'download_id' => $this->job_download_id];

            $result_array = ['my_design_id' => $my_design_id, 'download_id' => $this->job_download_id, 'is_limit_exceeded' => 0, 'result' => $image_list];
            $response = Response::json(['code' => 200, 'message' => 'Design generated successfully', 'cause' => '', 'data' => $result_array]);
            ...
            $failed = 2;
            $fail_reason = json_encode(['download_id' => $this->job_download_id, "\nerror_msg" => $e->getMessage()]);
            DB::beginTransaction();
        ...
        try {
            ...
            if ($counter <= 10) {
                ...
                if (count($job_result) > 0) {
                    ...
                    if ($is_dismiss) {
                        ...
                        if (count($CheckOldSql) > 0) {
                            ...
                            return Response::json(['code' => $http_code, 'message' => $msg, 'cause' => '', 'data' => $result]);
                        }
                    }
                } else {
                    $status = 2;
                ...
                if ($noderesponse != 200) {
                    ...
                    $response = Response::json(['code' => $http_code, 'message' => $msg, 'cause' => 'Node server has been stopped.', 'data' => $result]);
                } else {
    ...
    public function testNodeServer($download_id)
    {
        ...
        if ($response === false) {
            ...
            $email_array = ['annu.optimumbrew@gmail.com', 'dhrutip.optimumbrew@gmail.com', 'alagiyanirav@gmail.com'];
    ...
    public function getUrlFromNodeRoute(Request $request_body)
    {
        try {
            ...
            $response = Response::json(['code' => 200, 'message' => 'Success', 'cause' => '']);
        } catch (Exception $e) {
    ...
    public function getGenerateDesignReportForAdmin(Request $request_body)
    {
        try {
            ...
            $redis_result = ['total_record' => $total_record, 'is_next_page' => $is_next_page, 'result' => $result];

            $response = Response::json(['code' => 200, 'message' => 'Design report successfully received.', 'cause' => '', 'data' => $redis_result]);
    ...
    public function cancleLiveQueueJobForDesign(Request $request_body)
    {
        try {
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameterIsArray(['cd_ids'], $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Jobs cancelled successfully', 'cause' => '', 'data' => json_decode('{}')]);
