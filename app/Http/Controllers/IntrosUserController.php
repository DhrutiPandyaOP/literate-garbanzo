...
class IntrosUserController extends Controller
{
    ...
    public function generateIntrosVideo(Request $request_body)
    {
        try {
            ...
            if (($response = (new VerificationController())->validateRequiredParameter(['json_data', 'download_json', 'my_design_id', 'sub_category_id'], $request)) != '') {
            ...
            if ($my_design_id) {
                if (($response = (new VerificationController())->validateRequiredParameter(['sample_image', 'user_template_name'], $request)) != '') {
                    (new UserController())->addVideoGenerateHistory(json_decode(json_encode($response))->original->message, $user_id, null, null, $content_id, 3, null);
            ...
            if ($result['result_status'] == 0) {
                $response = Response::json(['code' => 201, 'message' => 'Video is unable to generate video', 'cause' => '', 'data' => json_decode('{}')]);
            } else {
                $download_id = $result['download_id'];
                $result_array = ['my_design_id' => $my_design_id, 'download_id' => $download_id, 'is_limit_exceeded' => 0, 'result' => $image_list];
                $response = Response::json(['code' => 200, 'message' => 'Video generated successfully', 'cause' => '', 'data' => $result_array]);
            }
