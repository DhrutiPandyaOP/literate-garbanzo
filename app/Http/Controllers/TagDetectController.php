...
class TagDetectController extends Controller
{
    public function getTagInImageByViaURL(Request $request_body)
    {
        try {
            ...
            if (($response = (new VerificationController())->validateRequiredParameter(['image_url'], $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'get tag in image by via URL.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
            DB::rollback();
        }
