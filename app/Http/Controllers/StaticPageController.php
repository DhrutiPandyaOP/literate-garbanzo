...
class StaticPageController extends Controller
{
  ...
  public function getStaticPageTemplateListById(Request $request_body)
  {
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('static_page_id'), $request)) != '')
        return $response;
...
//            $sub_category_lists[0]->is_selected=1;
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Template fetched successfully.', 'cause' => '', 'data' => $redis_result));
...
//      Log::error("getStaticPageTemplateListById : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get static page template list id.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
...
//          return array('active_path' => $ACTIVATION_LINK_PATH,'total_record' => $total_row, 'is_next_page' => $is_next_page, 'template_list' => $template_list);
          return array('active_path' => $ACTIVATION_LINK_PATH,'template_list' => $template_list);
        });
      }
      $this->search_category =isset($request->search_category) && trim($request->search_category) !='' ? trim(strtolower($request->search_category)) : "";
  ...
  */
  public function searchStaticPageTemplateBySubCategoryId(Request $request_body)
  {
    try {
      ...
      $request = json_decode($request_body->getContent());
      if (($response = (new VerificationController())->validateRequiredParameter(array('sub_category_id', 'page', 'item_count', 'content_type'), $request)) != '')
        return $response;
      ...
      if (!$redis_result['result']['total_record']) {
        $redis_result = [];
        $response = Response::json(array('code' => $this->default_code, 'message' => "Sorry, we couldn't find any templates for '$this->db_search_category'", 'cause' => '', 'data' => json_decode("{}")));
      }else{
        if($this->search_category != ''){
          if($this->page == 1){
            if($redis_result['code'] == 200){
    ...
    public function getTemplatesByCategoryId(Request $request_body)
    {
        try {
            ...
            $redis_result = Cache::remember("getTemplatesByCategoryId:$this->sub_category_id:$this->catalog_id:$this->content_id:$this->content_type:$this->page:$this->item_count", Config::get('constant.CACHE_TIME_7_DAYS'), function () {
                ...
                $is_next_page = ($total_row > ($this->offset + $this->item_count)) ? true : false;
                $result = array('total_record' => $total_row, 'is_next_page' => $is_next_page, 'result' => $template_list);
                return $result;
            });

            if (!$redis_result['result']) {
                $response = Response::json(array('code' => 201, 'message' => 'Template does not exist in this category.', 'cause' => '', 'data' => json_decode("{}")));
            } else {
                $response = Response::json(array('code' => 200, 'message' => 'All sub category are fetched successfully.', 'cause' => '', 'data' => $redis_result));
            }

        } catch (Exception $e) {
            (new ImageController())->logs("getTemplatesByCategoryId", $e);
            //Log::error("getTemplatesByCategoryId : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get static page template list admin', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
        }
        return $response;
    }
    ...
    */
    public function getTemplatesByCategoryIdV2BackUp(Request $request_body)
    {
      try {
        ...
        if (($response = (new VerificationController())->validateRequiredParameterIsArray(array('content_uuids'), $request)) != '')
          return $response;
        ...
        if (!$redis_result['result']) {
          $response = Response::json(array('code' => 201, 'message' => 'Template does not exist in this category.', 'cause' => '', 'data' => $redis_result));
        } else {
          $response = Response::json(array('code' => 200, 'message' => 'All sub category are fetched successfully.', 'cause' => '', 'data' => $redis_result));
        }
      ...
      if ($this->page <= 5) {
        ...
        if ($this->catalog_id) {
          ...
          $redis_result = Cache::rememberforever("getTemplatesByCategoryIdV2:$this->sub_category_id:$this->catalog_id:$this->content_type:$this->page:$this->item_count", function () {
            ...
            return array('total_record' => $total_row, 'is_next_page' => $is_next_page, 'result' => $template_list);
          });
        ...
        $template_list = [];
        $is_next_page = ($total_row > ($this->offset + $this->item_count));
        $redis_result = array('total_record' => $total_row, 'is_next_page' => $is_next_page, 'result' => $template_list);
      }
    ...
    try {
      ...
      if (!Cache::has("Config::get('constant.REDIS_KEY'):getStaticPageSubCategoryByAdmin")) {
        $result = Cache::rememberforever("getStaticPageSubCategoryByAdmin", function () {
          ...
          return array('sub_category_lists' => $sub_category_lists , 'total_record' => $total_row);
    ...
    try {
      ...
      $this->static_page_dir = Config::get('constant.ACTIVATION_LINK_PATH') . Config::get('constant.STATIC_PAGE_DIRECTORY');
        ...
        $redis_result = Cache::rememberforever("getVideoStaticPageTagListByAdmin$this->content_type:$this->page:$this->item_count", function () {
          ...
          $is_next_page = ($total_row > ($this->offset + $this->item_count)) ? true : false;
          return array('total_record' => $total_row, 'is_next_page' => $is_next_page,'tag_lists' => $tag_lists);
        });
      ...
      $response = Response::json(array('code' => 200, 'message' => 'All tag fetched successfully.', 'cause' => '', 'data' => $redis_result));
...
//      Log::error("getAllVideoStaticPageTagListByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
  ...
  public function setStaticPageRankByAdmin(Request $request_body)
  {
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredArrayParameter(array('static_page_ids','content_type'), $request)) != '')
        return $response;

      if (!is_array($request->static_page_ids) OR count($request->static_page_ids) == 0) {
        return $response = Response::json(array('code' => 201, 'message' => 'must be array or atleast one value', 'cause' => '', 'data' => json_decode("{}")));
      }
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Static Page Rank set successfully.', 'cause' => '', 'data' => json_decode("{}")));
    ...
    try {
      ...
      if (!Cache::has("Config::get('constant.REDIS_KEY'):getStaticPageCatalogListByAdmin$this->sub_category_id")) {
        $result = Cache::rememberforever("getStaticPageCatalogListByAdmin$this->sub_category_id", function () {
          ...
          return array('catalog_lists' => $catalog_lists);
        });
      }
...
//      Log::error("getStaticPageCatalogListByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function getStaticMainPageByAdmin(Request $request_body)
    {
        try {
            ...
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get main static page template list admin', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
  ...
  public function generateStaticPageByAdmin(Request $request_body)
  {
    ...
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('sub_category_id', 'sub_category_name', 'sub_category_path', 'page_url', 'page_detail', 'header_detail', 'sub_detail'), $request)) != '')
        return $response;
      ...
      $guide_heading="";
      $guide_btn_text="";
      $similar_tags = isset($request->similar_tags) && !empty($request->similar_tags) ? $request->similar_tags : array();
      $rating_schema = isset($request->rating_schema) ? $request->rating_schema : NULL;
      $guide_steps = isset($request->guide_steps) && !empty($request->guide_steps) ? $request->guide_steps : array();
      $faqs = isset($request->faqs) && !empty($request->faqs) ? $request->faqs : array();

      if($sub_category_path !='' && preg_match('/[A-Z]/', $sub_category_path)){
        return Response::json(array('code' => 201, 'message' => 'Please enter characters from a-z,0-9 and - only in `category name in url`.', 'cause' => '', 'data' => json_decode("{}")));
      }

      if($catalog_path !='' && preg_match('/[A-Z]/', $catalog_path)){
        return Response::json(array('code' => 201, 'message' => 'Please enter characters from a-z,0-9 and - only in `catalog name in url`.', 'cause' => '', 'data' => json_decode("{}")));
      }
      ...
      if(count($guide_steps) > 7 ){
        return Response::json(array('code' => 201, 'message' => 'You can only add 7 or fewer user guide step.', 'cause' => '', 'data' => json_decode("{}")));
      }

      if(count($faqs) > 8 ){
        return Response::json(array('code' => 201, 'message' => 'You can only add 8 or fewer FAQs.', 'cause' => '', 'data' => json_decode("{}")));
      }

      if(count($guide_steps) > 0 ){
          if (($response = (new VerificationController())->validateRequiredParameter(array('guide_heading', 'guide_btn_text'), $guide_detail)) != '')
            return $response;
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('page_title', 'meta', 'canonical'), $page_detail)) != '')
        return $response;
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('h1', 'h2', 'cta_text', 'cta_link'), $header_detail)) != '')
        return $response;
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('h2', 'description'), $sub_detail)) != '')
        return $response;
      ...
      if($rating_schema){
        if (($response = (new VerificationController())->validateRequiredParameter(array('userName','name', 'description', 'ratingValue', 'reviewCount'), $rating_schema)) != '')
              $rating_schema = NULL;
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Static page generated successfully.', 'cause' => '', 'data' => ['result' => $this->static_page_dir . $page_url]));
...
*/
    public function generateStaticPageByAdminV2(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(array('sub_category_id', 'sub_category_name', 'sub_category_path', 'page_url', 'page_detail', 'header_detail', 'sub_detail', 'app_cta_detail'), $request)) != '')
                return $response;

            if(($response = (new VerificationController())->validateRequiredParameterIsArray(array('content_ids', 'template_list'), $request)) != '')
                return $response;
            ...
            $template = array(
                ...
                'video_template_list' => json_encode(array()),
  ...
  public function generateVideoStaticPageByAdmin(Request $request_body)
  {
    ...
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('tag_title','search_category','page_url', 'page_detail', 'header_detail', 'sub_detail'), $request)) != '')
        return $response;
...
//      $catalog_path = isset($request->catalog_path) ? $request->catalog_path : NULL;
      ...
      if(preg_match('/[A-Z]/', $page_url)){
        return Response::json(array('code' => 201, 'message' => 'Please enter characters from a-z,0-9 and - only in `page name in url`.', 'cause' => '', 'data' => json_decode("{}")));
      }
...
//      Log::error("generateVideoStaticPageByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function generateVideoStaticPageByAdminV2(Request $request_body)
    {
        try {
            ...
            if (($response = (new VerificationController())->validateRequiredParameter(array('tag_title', 'search_category', 'page_url', 'page_detail', 'header_detail', 'sub_detail', 'search_category', 'app_cta_detail'), $request)) != '')
                return $response;
...
//        return $response;
            ...
            $template = array(
                ...
                'sub_category_id' => "",
                'API_getTemplatesByCategoryIdV2' => $API_getTemplatesByCategoryIdV2,
                'image_template_list' => json_encode(array()),
  ...
  public function editStaticPageByAdmin(Request $request_body)
  {
    ...
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('static_page_id', 'sub_category_id', 'sub_category_name', 'sub_category_path', 'old_sub_category_path', 'old_page_url', 'page_url', 'page_detail', 'header_detail', 'sub_detail'), $request)) != '')
        return $response;
      ...
      if (file_exists($old_file_dir)) {
        ...
        if ($sub_category_path != $old_sub_category_path) {
          ...
          if (!rename($static_page_dir . $old_sub_category_path, $static_page_dir . '/' . $sub_category_path)) {
            return Response::json(array('code' => 201, 'message' => 'Sorry, We couldn\'t rename url of sub category path.', 'cause' => '', 'data' => json_decode("{}")));
          }
        }
      ...
      if (!rename($file_dir, $old_file_rename)) {
        return Response::json(array('code' => 201, 'message' => 'Sorry, We couldn\'t generate this page. Try again.', 'cause' => '', 'data' => json_decode("{}")));
      }
...
//      }
      ...
      $handle = fopen($file_dir, 'w') or die('Cannot open file');
...
//      Log::error("editStaticPageByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'edit static page.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
  ...
  */
    public function editStaticPageByAdminV2(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(array('static_page_id', 'sub_category_id', 'sub_category_name', 'sub_category_path', 'old_sub_category_path', 'old_page_url', 'page_url', 'page_detail', 'header_detail', 'sub_detail', 'app_cta_detail'), $request)) != '')
                return $response;
...
//      }
    ...
    public function editStaticMainPageByAdmin(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(array('content_ids', 'page_detail', 'header_detail', 'sub_detail', 'app_cta_detail'), $request)) != '')
                return $response;
            ...
            $content_type = "";
            $guide_steps = isset($request->guide_steps) && !empty($request->guide_steps) ? json_decode(json_encode($request->guide_steps), true) : array();
            $faqs = isset($request->faqs) && !empty($request->faqs) ? json_decode(json_encode($request->faqs), true) : array();
            ...
            if (($response = (new VerificationController())->validateRequiredParameter(array('h1', 'h2', 'cta_text'), $header_detail)) != '')
                return $response;
            ...
            $handle = fopen($static_page_dir . $file_name, 'w') or die('Cannot open file');
            ...
            $response = Response::json(array('code' => 200, 'message' => 'Static landing page generated successfully.', 'cause' => '', 'data' => ['result' => $static_page_dir . $file_name]));

        } catch (Exception $e) {
            (new ImageController())->logs("editStaticMainPageByAdmin", $e);
            //Log::error("editStaticMainPageByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'edit static landing page.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
            DB::rollBack();
        }
        return $response;
    }
  ...
  public function editVideoStaticPageByAdmin(Request $request_body)
  {
    ...
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('static_page_id','tag_title','search_category','old_page_url','page_url', 'page_detail', 'header_detail', 'sub_detail'), $request)) != '')
        return $response;
...
//      Log::error("editVideoStaticPageByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function editVideoStaticPageByAdminV2(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(array('static_page_id', 'tag_title', 'search_category', 'old_page_url', 'page_url', 'page_detail', 'header_detail', 'sub_detail', 'app_cta_detail'), $request)) != '')
                return $response;
...
//      Log::error("setStaticPageOnTheTopByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'set static page sequence.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
    }
    return $response;
  }
  ...
  public function setStaticPageOnTheTopByAdmin(Request $request_body)
  {
    try {
      ...
      if ($oldest_static_page) {
        ...
        $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'set static page sequence.', 'cause' => '', 'data' => json_decode("{}")));
      }
  ...
  public function setStatusOfStaticPage(Request $request_body)
  {
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('static_page_id', 'status'), $request)) != '')
        return $response;
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Status set successfully.', 'cause' => '', 'data' => json_decode("{}")));

    } catch (Exception $e) {
      (new ImageController())->logs("setStatusOfStaticPage",$e);
//      Log::error("setStatusOfStaticPage : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'set status of static page.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
    }
    return $response;
  }
...
//            Log::info('sub category list : ',[$sub_categories]);
          ...
          return array('sub_categories' => $sub_categories, 'catalogs' => $catalogs);
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Sub categories are fetched successfully.', 'cause' => '', 'data' => ['result' => $redis_result]));
      $response->headers->set('Cache-Control', Config::get('constant.RESPONSE_HEADER_CACHE'));
...
//      Log::error("getAllSubCategoryListForStaticPage : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . ' get all sub category for static page.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    }
    return $response;
  }
  ...
  public function getAllTemplateListForMainStaticPage(Request $request_body)
  {
    try {
      ...
      $request = json_decode($request_body->getContent());
      if (($response = (new VerificationController())->validateRequiredParameter(array('content_type', 'page', 'item_count'), $request)) != '')
        return $response;
      ...
      $redis_result = Cache::rememberforever("getAllTemplateListForMainStaticPage:$this->page:$this->item_count:$this->content_type", function () {
        ...
        $is_next_page = ($total_row > ($this->offset + $this->item_count)) ? true : false;
        return array('total_record' => $total_row, 'is_next_page' => $is_next_page, 'content_list' => $template_list);
      ...
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get static main page template list admin', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
  ...
  public function getWhatsNewContent(Request $request_body)
  {
    ...
    try {
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Get What\'s new content successfully.', 'cause' => '', 'data' => ['result' =>$redis_result]));

    } catch (Exception $e) {
      (new ImageController())->logs("getWhatsNewContent",$e);
//      Log::error("getWhatsNewContent : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get Whats\'s new content.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
  ...
  public function uploadMediaForWhatsNew(Request $request_body)
  {
    ...
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('file_type'), $request)) != '')
        return $response;
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Media uploaded successfully.', 'cause' => '', 'data' => ['result' =>$files]));

    } catch (Exception $e) {
      (new ImageController())->logs("uploadMediaForWhatsNew",$e);
//      Log::error("uploadMediaForWhatsNew : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'media uploaded.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
  ...
  public function saveWhatNewHtml(Request $request_body)
  {
    try {
      ...
      $request = json_decode($request_body->getContent());
      if (($response = (new VerificationController())->validateRequiredParameter(array('html_block','content_json'), $request)) != '')
        return $response;
...
//      Log::info('whats new index :',[$file_dir]);
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Save what\'s new successfully.', 'cause' => '', 'data' => ['result' => $this->static_page_dir]));

    } catch (Exception $e) {
      (new ImageController())->logs("saveWhatNewHtml",$e);
//      Log::error("saveWhatNewHtml : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'save what\'s new.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
  ...
  public function deleteWhatsNewMedia(Request $request_body)
  {
    try {
      ...
      if(count($media) == 0){
        return Response::json(array('code' => 201, 'message' => 'Media file does not found.', 'cause' => '', 'data' => json_decode("{}")));
      }
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Delete what\'s new media successfully.', 'cause' => '', 'data' => json_decode("{}")));

    } catch (Exception $e) {
      (new ImageController())->logs("deleteWhatsNewMedia",$e);
//      Log::error("deleteWhatsNewMedia : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'delete what\'s new media.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
  ...
  public function getWhatsNewHtmlBlocks(Request $request_body)
  {
    try {
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Get Whats\'s new html blocks successfully.', 'cause' => '', 'data' => ['html_block' => $redis_result]));

    } catch (Exception $e) {
      (new ImageController())->logs("getWhatsNewHtmlBlocks",$e);
//      Log::error("getWhatsNewHtmlBlocks : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get Whats\'s new blocks.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
    ...
    try{
      ...
      if (!Cache::has("Config::get('constant.REDIS_KEY'):getLeftNavigation:$this->page_id")) {
        $result = Cache::rememberforever("getLeftNavigation:$this->page_id", function () {
          ...
          $all_pages = array('image_pages' => $image_pages, 'video_pages' => $video_pages);
...
//          $left_nav_html= file_get_contents($file_path);
      ...
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get left navigation bar.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    ...
    public function getLeftNavigationV2($image_class_name = '', $video_class_name = '', $static_page_id = '')
    {
        try {
            ...
            $response = Cache::rememberforever("getLeftNavigationV2:$this->page_id:$this->image_class_name:$this->video_class_name", function () {
                ...
                $all_pages = array('image_pages' => $image_pages, 'video_pages' => $video_pages, 'image_class_name' => $this->image_class_name, 'video_class_name' => $this->video_class_name);
...
//            }
      ...
      if($this->cache){
        $result = array("generated_pages"=>$page_count);
        $message ='Static pages generated successfully.';
      }else{
        $result = array("generated_pages"=>0);
...
//                $template_list = $this->getStaticPageTemplateListByContentIds($content_ids);
                ...
                foreach ($pages AS $i => $row) {
                    ...
                    $guide_heading = isset($guide_detail->guide_heading) ? $guide_detail->guide_heading :"";
                    $guide_btn_text = isset($guide_detail->guide_btn_text) ? $guide_detail->guide_btn_text :"";
                    $image_template_list = array();
                    $video_template_list = array();
    ...
    try {
      ...
      if (!Cache::has("Config::get('constant.REDIS_KEY'):getAllStaticPageURL")) {
        $result = Cache::rememberforever("getAllStaticPageURL", function () {
          ...
          return array("image_pages" => $image_pages, "video_pages" => $video_pages);
        });
      }
      ...
      $response = Response::json(array('code' => 200, 'message' => 'URL fetch successfully.', 'cause' => '', 'data' => array("total_pages" => $total_pages, "image_page" => $image_page, "video_page" => $video_page, "intro_page" => $intro_page)));

    } catch (Exception $e) {
      (new ImageController())->logs("getAllStaticPageURL",$e);
//      Log::error("getAllStaticPageURL : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'URL fetch', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    ...
    try {
      ...
      foreach ($decoded_json as $key) {
        if (in_array($keyword, $key->keywords)) {
          return array('cta_text' => $key->cta_text, 'destination' => $key->destination);
        }
      }
      ...
      foreach ($decoded_json as $value) {
        ...
        if (preg_match('(' . $array_diff . ')', $keyword) === 1) {
          return array('cta_text' => $value->cta_text, 'destination' => $value->destination);
        }
      }
      return array('cta_text' => 'Create Your Design Now', 'destination' => (object)["playStoreLink" => "https://play.google.com/store/apps/details?id=com.oneintro.intromaker&referrer=utm_source%3DOB_PAK", "appStoreLink" => "https://apps.apple.com/us/app/intro-maker-outro-maker/id1516421168"]);
  ...
  public function getPageFromTag(Request $request_body)
  {
    try {
      ...
      if(strlen($this->tag_name) >= 50 || $this->tag_name == "") {
        return Response::json(array('code' => 201, 'message' => 'Invalid search text please enter valid search text.', 'cause' => '', 'data' => json_decode("{}")));
      }
        ...
        $result = Cache::rememberforever("getPageFromTag:$this->tag_name:$this->content_type", function () {
          ...
          if($this->content_type == Config::get('constant.IMAGE')) {
            ...
            $sub_pages = DB::select('SELECT
                                      ...
                                      ORDER BY sp.update_time DESC');
          }else if($this->content_type ==  Config::get('constant.VIDEO')){
            $main_pages = DB::select('SELECT
                                        sp.id,
                                        CONCAT("' . $this->static_page_dir . '",sp.tag_in_url,"/") AS page_url
                                      FROM static_page_master AS sp
      ...
      if(count($redis_result) > 0 ){
        $response = Response::json(array('code' => 200, 'message' => 'Pages fetched successfully.', 'cause' => '', 'data' =>$redis_result));
      }else{
        $response = Response::json(array('code' => 201, 'message' => 'No pages found of this tag.', 'cause' => '', 'data' =>json_decode("{}")));
      }

    } catch (Exception $e) {
      (new ImageController())->logs("getPageFromTag",$e);
//      Log::error("getPageFromTag : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get page.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
    }
    return $response;
  }
...
//      Log::error("deleteSimilarTemplatePage : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
  ...
  public function updateSimilarTemplatePage(Request $request_body)
  {
    try {
      ...
      if (($response = (new VerificationController())->validateRequiredParameter(array('id', 'tag_name','page_type','page_url_id','static_page_id'), $request)) != '')
        return $response;
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Similar tag updated successfully.', 'cause' => '', 'data' => array("tag"=>$tag_name)));
...
//      Log::error("updateSimilarTemplatePage : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
  ...
  public function deleteSimilarTemplatePage(Request $request_body)
  {
    try {
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Similar tag deleted successfully.', 'cause' => '', 'data' => json_decode('{}')));
    } catch (Exception $e) {
      (new ImageController())->logs("deleteSimilarTemplatePage",$e);
//      Log::error("deleteSimilarTemplatePage : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'delete similar tag.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
      DB::rollBack();
    }
    return $response;
  }
...
//            $redis_result = Cache::remember("getStaticPageTemplateListByContentIds$this->content_type:$this->content_ids:$this->page:$this->item_count", Config::get('constant.CACHE_TIME_7_DAYS'), function () {
                ...
                return array('template_list' => $template_list);
//            });
//
//            return $redis_result;
            ...
            return array('template_list' => array());
        }
    }
  ...
  public function getTemplateDetailsBySubCategoryId(Request $request_body)
  {
    try {
      ...
      if ($content_details) {
        ...
        return Response::json(array('code' => 201, 'message' => 'Template doesn\'t exist.', 'cause' => '', 'data' => json_decode("{}")));
      }
      $response = Response::json(array('code' => 200, 'message' => 'Template details get successfully.', 'cause' => '', 'data' => ['content_details' => $content_details]));

    } catch (Exception $e) {
      (new ImageController())->logs("getTemplateDetailsBySubCategoryId", $e);
      $response = Response::json(array('code' => 201, 'message' => config('constant.EXCEPTION_ERROR') . 'get templates.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    }
    return $response;
  }
