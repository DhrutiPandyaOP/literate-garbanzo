    ...
    */
    ...
    public function addPostSuggestionByAdmin(Request $request_body)
    {
        ...
        try {
            ...
            if (($response = (new VerificationController())->validateRequiredParameter(['name', 'title', 'preview_content_id', 'tag', 'short_description', 'long_description'], $request)) != '') {
                return $response;
            }
            ...
            $related_tag = isset($request->related_tag) && ! empty($request->related_tag) ? $request->related_tag : [];
            ...
            if (is_array($template_content_ids) and count($template_content_ids) > 0 and count($template_content_ids) <= $max_template_allow) {
                ...
                return $response = Response::json(['code' => 201, 'message' => 'The post template should not exceed the max length than '.$max_template_allow.' templates Or it accept only array', 'cause' => '', 'data' => json_decode('{}')]);
            }

            $max_title_length = Config::get('constant.MAX_LENGTH_FOR_POST_SUGGESTION_TITLE');
            if ($max_title_length < strlen($title)) {
                return $response = Response::json(['code' => 201, 'message' => 'The event title should not exceed the max length than '.$max_title_length.'.', 'cause' => '', 'data' => json_decode('{}')]);
            }

            $max_name_length = Config::get('constant.MAX_LENGTH_FOR_POST_SUGGESTION_NAME');
            if ($max_name_length < strlen($name)) {
                return $response = Response::json(['code' => 201, 'message' => 'The event name should not exceed the max length than '.$max_name_length.'.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            if (count($is_exist) > 0) {
                return Response::json(['code' => 201, 'message' => 'This post title or name already exist.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            if (($response = (new VerificationController())->verifySearchText($search_text_tag)) != 1) {
                return $response = Response::json(['code' => 201, 'message' => 'Invalid tag name. Please enter valid tag name.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post suggestion added successfully.', 'cause' => '', 'data' => json_decode('{}')]);

        } catch (Exception $e) {
            (new ImageController())->logs('addPostSuggestionByAdmin', $e);
            //            Log::error("addPostSuggestionByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'add post suggestion by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
            DB::rollBack();
        }
    ...
    */
    ...
    public function updatePostSuggestionByAdmin(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['post_suggestion_id', 'title', 'preview_content_id', 'tag', 'short_description', 'long_description'], $request)) != '') {
                return $response;
            }
            ...
            if (is_array($template_content_ids) and count($template_content_ids) > 0 and count($template_content_ids) <= $max_template_allow) {
                ...
                return $response = Response::json(['code' => 201, 'message' => 'The post template should not exceed the max length than '.$max_template_allow.' templates Or it requires an array', 'cause' => '', 'data' => json_decode('{}')]);
            }

            $max_title_length = Config::get('constant.MAX_LENGTH_FOR_POST_SUGGESTION_TITLE');
            if ($max_title_length < strlen($title)) {
                return $response = Response::json(['code' => 201, 'message' => 'The post title should not exceed the max length than '.$max_title_length.'.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post suggestion updated successfully.', 'cause' => '', 'data' => json_decode('{}')]);

        } catch (Exception $e) {
            (new ImageController())->logs('updatePostSuggestionByAdmin', $e);
            //            Log::error("updatePostSuggestionByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'update post suggestion by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
            DB::rollBack();
        }
    ...
    */
    ...
    public function deletePostSuggestionByAdmin(Request $request_body)
    {
        ...
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['post_suggestion_id'], $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post suggestion deleted successfully.', 'cause' => '', 'data' => json_decode('{}')]);

        } catch (Exception $e) {
            (new ImageController())->logs('deletePostSuggestionByAdmin', $e);
            //            Log::error("deletePostSuggestionByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'delete post suggestion by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
            DB::rollBack();
        }
    ...
    */
    ...
    public function getPostSuggestionByAdmin(Request $request_body)
    {
        try {
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post suggestion fetched successfully.', 'cause' => '', 'data' => $redis_result]);
            ...
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'fetched post suggestion by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
    ...
    */
    ...
    public function setPostSuggestionRankOnTheTopByAdmin(Request $request_body)
    {
        try {
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post suggestion rank set successfully.', 'cause' => '', 'data' => json_decode('{}')]);
            ...
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'set post suggestion rank by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
    ...
    */
    ...
    public function setPostScheduleByAdmin(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['post_date'], $request)) != '') {
                return $response;
            }

            if (($response = (new VerificationController())->validateRequiredArrayParameter(['post_ids'], $request)) != '') {
                return $response;
            }
            ...
            if (! is_array($post_ids)) {
                return $response = Response::json(['code' => 201, 'message' => 'The post event id is accept only array', 'cause' => '', 'data' => json_decode('{}')]);
            }

            if (count($post_ids) !== count(array_unique($post_ids))) {
                return Response::json(['code' => 201, 'message' => 'Duplicate post are not allowed at same day.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            if (count($post_ids) <= 0 or count($post_ids) > $max_post_to_schedule) {
                return $response = Response::json(['code' => 201, 'message' => 'The post event should be between 1 and '.$max_post_to_schedule.' .', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            if (! checkdate($month, $day, $year)) {
                return Response::json(['code' => 201, 'message' => 'Invalid date format or Invalid date. Please enter the date in the format "YYYY-MM-DD".', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            if (count($is_exist) > 0) {
                return Response::json(['code' => 201, 'message' => 'Post date already exist.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post schedule set successfully.', 'cause' => '', 'data' => ['post_schedule_id' => $post_schedule_id]]);

        } catch (Exception $e) {
            (new ImageController())->logs('setPostScheduleByAdmin', $e);
            //            Log::error("setPostScheduleByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'set post schedule by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
            DB::rollBack();
        }
    ...
    */
    ...
    public function deletePostScheduleByAdmin(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['post_schedule_id'], $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post schedule deleted successfully.', 'cause' => '', 'data' => json_decode('{}')]);

        } catch (Exception $e) {
            (new ImageController())->logs('deletePostScheduleByAdmin', $e);
            //            Log::error("deletePostScheduleByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'delete post schedule by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
            DB::rollBack();
        }
    ...
    */
    ...
    public function getPostScheduleDetailByAdmin(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['month', 'year'], $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Post scheduler fetched successfully.', 'cause' => '', 'data' => ['result' => $redis_result]]);
            //$response->headers->set('Cache-Control', Config::get('constant.RESPONSE_HEADER_CACHE'));

        } catch (Exception $e) {
            (new ImageController())->logs('getPostScheduleDetailByAdmin', $e);
            //            Log::error("getPostScheduleDetailByAdmin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'fetched post scheduler by admin.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
    ...
    */
    public function getAllTemplateBySearchTag(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['search_category', 'page', 'item_count'], $request)) != '') {
                return $response;
            }
            ...
            if (! Cache::has("Config::get('constant.REDIS_KEY'):getAllTemplateBySearchTag:$this->search_category")) {
                $result = Cache::rememberforever("getAllTemplateBySearchTag:$this->search_category", function () {
                    ...
                    return ['total_record' => $total_row, 'result' => $search_result];
            ...
            if (! $redis_result) {
                ...
                $response = ['code' => 201, 'message' => $message, 'cause' => '', 'data' => json_decode('{}')];
            } else {
                ...
                $response = ['code' => $code, 'message' => $message, 'cause' => '', 'data' => $redis_result];
            }
    ...
    public function getPostScheduleListForPreview(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['event_name'], $request)) != '') {
                return $response;
            }
            ...
            if (! Cache::has("Config::get('constant.REDIS_KEY'):getPostScheduleListForPreview$this->event_name")) {
                $result = Cache::rememberforever("getPostScheduleListForPreview$this->event_name", function () {
                    ...
                    return ['event_detail' => $event_detail, 'template_list' => $template_list, 'more_template_url' => $more_template_url];
            ...
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'get post schedule.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
            ...
            if (! Cache::has("Config::get('constant.REDIS_KEY'):getPostScheduleList$this->event_date:$this->event_name")) {
                $result = Cache::rememberforever("getPostScheduleList$this->event_date:$this->event_name", function () {
                    ...
                    if (($this->event_date) && (! $this->event_name)) {
                        ...
                        return ['date_heading' => $this->date_heading, 'result' => $post_schedules];
                    } else {
                        ...
                        return ['prev_event_details' => $prev_event_details, 'next_event_details' => $next_event_details, 'event_detail' => $event_detail, 'template_list' => $template_list, 'date_heading' => $this->date_heading, 'more_template_url' => $more_template_url];
    ...
    public function updateRelatedTags(Request $request_body)
    {
        try {
            ...
            if (($response = (new VerificationController())->validateRequiredParameter(['id', 'tag_name', 'page_url', 'event_id'], $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Related tag updated successfully.', 'cause' => '', 'data' => ['tag' => $tag_name]]);
    ...
    public function deleteRelatedTags(Request $request_body)
    {
        try {
            ...
            $response = Response::json(['code' => 200, 'message' => 'Related tag deleted successfully.', 'cause' => '', 'data' => json_decode('{}')]);
        } catch (Exception $e) {
            (new ImageController())->logs('deleteRelatedTags', $e);
            //            Log::error("deleteRelatedTags : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'delete related tag.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
            DB::rollBack();
        }
