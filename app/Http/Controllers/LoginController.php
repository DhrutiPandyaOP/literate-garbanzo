...
class LoginController extends Controller
{
    ...
    public function doLoginForAdmin(Request $request_body)
    {
        try {
          ...
          if (in_array("$user_id", array($admin, $sub_admin), true) || in_array($role_id[0]->role_id, array($crm_user_role_id)))
            {
              ...
              if ($google2fa_enable == 1 && !isset($_COOKIE[$user_uuid])) {
                ...
                $response = Response::json(array('code' => 200, 'message' => 'Login successfully.', 'cause' => '', 'data' => ['token' => "", 'user_detail' => $user_details, 'role' => $role_id[0]->role_id]));

              } elseif ($google2fa_enable != 1) {
                $this->createNewSession($user_id, $token);
                $response = Response::json(array('code' => 200, 'message' => 'Login successfully.', 'cause' => '', 'data' => ['token' => $token, 'user_detail' => $user_details, 'role' => $role_id[0]->role_id]));
...
//      return $response;
      ...
      if (count($result) == 0) {
        return $response = Response::json(array('code' => 201, 'message' => 'You are inactive user. Please contact administrator.', 'cause' => '', 'data' => json_decode("{}")));
      }
...
//            Log::error('doLoginForUser : ', ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function doLoginForUser(Request $request_body)
    {
      try {
        ...
        if (($response = (new VerificationController())->validateRequiredParameter(array('email_id', 'password', 'device_info'), $request)) != '')
          return $response;
        ...
        if (!filter_var($email_id, FILTER_VALIDATE_EMAIL)) {
          ...
          return Response::json(array('code' => 201, 'message' => 'E-mail is in the wrong format.', 'cause' => '', 'data' => ''));
        }
        ...
        /* Check if user does social and email login both */
          ...
          if ($social_user[0]->signup_type == 2) {
            return Response::json(array('code' => 201, 'message' => 'You have done the signup using your Facebook account. If you want to login with email then first you have to set your password or you can use forgot password option.', 'cause' => '', 'data' => json_decode("{}")));
          } elseif ($social_user[0]->signup_type == 3) {
            return Response::json(array('code' => 201, 'message' => 'You have done the signup using your Google account. If you want to login with email then first you have to set your password or you can use forgot password option.', 'cause' => '', 'data' => json_decode("{}")));
          }
        /* Check user is active or not  */
        } elseif (!$social_user[0]->is_active) {
          return Response::json(array('code' => 201, 'message' => 'Your account has been deactivated.', 'cause' => '', 'data' => json_decode("{}")));
        }
    ...
    public function doLoginForSocialUserBackUp(Request $request)
    {
        try {
            ...
            $response = (new VerificationController())->validateRequiredParameter(array('social_uid', 'signup_type', 'device_info'), $request);
            if ($response != '')
                return $response;
            ...
            if (count($exist_id) > 0) {
                ...
                if (count($active_user) == 1) {
                    ...
                    $response = Response::json(array('code' => 200, 'message' => 'Login successfully.', 'cause' => '', 'data' => ['token' => $token, 'user_detail' => $user_detail]));
                ...
                $response = (new VerificationController())->validateRequiredParameter(array('first_name', 'email_id'), $request);
                if ($response != '') {
                    return $response;
                ...
                $data = array('email_id' => $email_id, 'request_json' => json_encode($request));
                ...
                if (count($active_user) == 1) {
                    ...
                    $response = Response::json(array('code' => 200, 'message' => 'User registered successfully.', 'cause' => '', 'data' => ['token' => $token, 'user_detail' => $user_detail]));
...
//            Log::error("doLoginForSocialUser : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function doLoginForSocialUser(Request $request)
    {
      try {
        $request = json_decode($request->getContent());
        /* Required request data parameter */
        if (($response = (new VerificationController())->validateRequiredParameter(array('signup_type', 'device_info'), $request)) != '') {
        ...
        if (!is_array($user_details)) {
          return Response::json(array('code' => 201, 'message' => config('constant.EXCEPTION_ERROR') . 'login.', 'cause' => '', 'data' => json_decode("{}")));
        }
        ...
        if ($exist_id) {
          ...
          if (!$exist_id[0]->is_active) {
            return Response::json(array('code' => 201, 'message' => 'You are inactive user. Please contact administrator.', 'cause' => '', 'data' => json_decode("{}")));
          }
        ...
        /* Get user details by user_id */
        $user_detail = (new LoginController())->getUserInfoByUserId($user_id);
        $response = Response::json(array('code' => 200, 'message' => $message, 'cause' => '', 'data' => ['token' => $token, 'user_detail' => $user_detail]));
        ...
        if ($e instanceof QueryException) {
          ...
          if ($errorCode == 1062) {
            ...
            return Response::json(array('code' => 201, 'message' => config('constant.EXCEPTION_ERROR') . 'login, Please try again later', 'cause' => '', 'data' => json_decode("{}")));
          }
        }
        (new ImageController())->logs("doLoginForSocialUser", $e);
        $response = Response::json(array('code' => 201, 'message' => config('constant.EXCEPTION_ERROR') . 'login.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
        DB::rollBack();
      }
      return $response;
    }
    ...
    public function checkSocialUserExist(Request $request)
    {
        try {
            ...
            if (count($exist_id) > 0) {
                ...
                $response = Response::json(array('code' => 421, 'message' => "Couldn't find your PhotoADKing account.", 'cause' => '', 'data' => json_decode("{}")));
...
//            Log::error("checkSocialUserExist : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function forgotPassword(Request $request_body)
    {
        try {
            ...
            if ($response == '')
                return Response::json(array('code' => 201, 'message' => "Couldn't find your email address.", 'cause' => '', 'data' => json_decode("{}")));
...
//              return Response::json(array('code' => 201, 'message' => 'You have done the signup using your social account like Facebook or Gmail. Please try to login using Facebook or Gmail.', 'cause' => '', 'data' => json_decode("{}")));
            ...
            DB::commit();
            $response = Response::json(array('code' => 200, 'message' => 'Reset password link has been sent to your email address.', 'cause' => '', 'data' => json_decode("{}")));
...
//            Log::error("forgotPassword : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'forgot Password.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
            DB::rollBack();
        }
        return $response;
    }
    ...
    public function verifyResetPasswordLink(Request $request_body)
    {
        try {
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(array('token', 'email_id'), $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(array('code' => 200, 'message' => 'Link verified successfully.', 'cause' => '', 'data' => json_decode("{}")));


        } catch (Exception $e) {
          (new ImageController())->logs("verifyResetPasswordLink",$e);
//            Log::error("verifyResetPasswordLink :", ["Exception" => $e->getMessage(), "\nTraceAsString :" => $e->getTraceAsString()]);
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'verify reset password link.', 'cause' => '', 'data' => json_decode("{}")));
            DB::rollBack();
        }
        return $response;
    }
    ...
    public function resetPassword(Request $request_body)
    {
        try {
            ...
            if($request->new_password != $string){
              return Response::json(array('code' => 201, 'message' => 'Password must be contain only alphabets,numeric and special character.', 'cause' => '', 'data' => ''));
            }
            ...
            if (count($result) > 0) {
                ...
                $response = Response::json(array('code' => 200, 'message' => 'Password set successfully.', 'cause' => '', 'data' => json_decode("{}")));
                ...
                $response = Response::json(array('code' => 201, 'message' => 'Please enter valid credential.', 'cause' => '', 'data' => json_decode("{}")));
            }
...
//            Log::error("resetPassword : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'reset password.', 'cause' => '', 'data' => json_decode("{}")));
            DB::rollBack();
        }
        return $response;
    }
  ...
  public function doUserAllSessionLogout(Request $request_body)
  {
    try {
      ...
      $request = json_decode($request_body->getContent());
      if (($response = (new VerificationController())->validateRequiredParameter(array('user_id'), $request)) != '')
        return $response;
...
//      }
      ...
      DB::commit();
      $response = Response::json(array('code' => 200, 'message' => 'User all session have successfully logged out.', 'cause' => '', 'data' => json_decode("{}")));
...
//            Log::error("changePassword : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
  ...
  public function setPassword(Request $request)
  {
    try {
      ...
      $request = json_decode($request->getContent());
      /* Required request data parameter */
      if (($response = (new VerificationController())->validateRequiredParameter(array('new_password'), $request)) != '') {
        return $response;
      }
      ...
      $response = Response::json(array('code' => 200, 'message' => 'Password changed successfully.', 'cause' => '', 'data' => ['token' => $new_token, 'is_multi_login' => 1]));
    ...
    public function setValueOfFirstTimeLogin()
    {
      try {
        ...
        DB::commit();
        $response = Response::json(array('code' => 200, 'message' => 'Value set successfully.', 'cause' => '', 'data' => json_decode("{}")));

      } catch (Exception $e) {
        (new ImageController())->logs("setValueOfFirstTimeLogin",$e);
//        Log::error("setValueOfFirstTimeLogin : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
        $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'set value of first time login.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
        DB::rollBack();
      }
      return $response;
    }
...
//            Log::error("getUserInfoByUserId : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function getUserDetailsByUserId($user_id)
    {
        try {
            ...
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'fetch user name & email_id.', 'cause' => '', 'data' => json_decode("{}")));
        }
        return $response;
    }
...
//            Log::error("addNewDeviceToUser : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function getPaymentsExpiryTime($user_id, $role_id)
    {
        try {
            ...
            $response = array('is_new_rule_applied' => $is_new_rule_applied, 'final_expiration_time' => $final_expiration_time, 'next_billing_date' => $next_billing_date, 'is_subscribe' => $is_subscribe, 'payment_type' => $payment_type);

        } catch (Exception $e) {
          (new ImageController())->logs("getPaymentsExpiryTime",$e);
//            Log::error("getPaymentsExpiryTime : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = array('is_new_rule_applied' => 1, 'final_expiration_time' => '', 'next_billing_date' => '', 'is_subscribe' => 2);
        }
        return $response;
    }
    ...
    try {
      ...
      if ($signup_type == 2) {
        ...
        if ($user_details) {
          ...
          return array('email' => $fb_email, 'social_uid' => $fb_social_uid, 'first_name' => $first_name, 'last_name' => $last_name);
        }
        ...
        if ($user_details) {
          ...
          return array('email' => $google_email, 'social_uid' => $google_social_uid, 'first_name' => $first_name, 'last_name' => $last_name);
        }
        Log::error('socialUserDetail : unable to get social user information.', ['response' => $response]);
        return $response = '';

      } else {
        return $response = Response::json(array('code' => 201, 'message' => 'Invalid signup type for social login.', 'cause' => '', 'data' => json_decode("{}")));
      }
    } catch (RequestException  $e) {
      (new ImageController())->logs("socialUserDetail", $e);
      $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get social user information.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    }
    return $response;
  }
