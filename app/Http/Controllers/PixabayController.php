...
class PixabayController extends Controller
{
    ...
    public function getImagesFromPixabay(Request $request_body)
    {
        try {
            ...
            if (strlen($search_query) > 100) {
                return Response::json(['code' => 201, 'message' => 'The length of your search is too long.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            if ($result === 429 or $result === 503 or $result === 201) {
                return Response::json(['code' => 201, 'message' => 'The server is unable to load images. Please try again after 10-20 minutes.', 'cause' => '', 'data' => json_decode('{}')]);
            }
            ...
            if ($result === 427) {
                ...
                return Response::json(['code' => 201, 'message' => $msg, 'cause' => '', 'data' => json_decode('{}')]);
            }

            $response = Response::json(['code' => 200, 'message' => 'Images fetched successfully.', 'cause' => '', 'data' => $result]);
            ...
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'get images from pixabay.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
    ...
    public function getVideosFromPixabay(Request $request_body)
    {
        try {
            ...
            if ($result === 429) {
                ...
                return Response::json(['code' => 201, 'message' => 'The server is unable to load videos. Please try again after 10-20 minutes.', 'cause' => '', 'data' => json_decode('{}')]);
            }

            $response = Response::json(['code' => 200, 'message' => 'Videos fetched successfully.', 'cause' => '', 'data' => $result]);
            $response->headers->set('Cache-Control', Config::get('constant.RESPONSE_HEADER_CACHE'));
            ...
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'get videos from pixabay.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
    ...
    public function uploadPixabayVideo(Request $request_body)
    {
        try {
            ...
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(['pixabay_video_url', 'pixabay_id'], $request)) != '') {
                return $response;
            }
            ...
            $response = Response::json(['code' => 200, 'message' => 'Video uploaded successfully.', 'cause' => '', 'data' => $stock_video_full_path.$file_name]);

        } catch (Exception $e) {
            (new ImageController())->logs('uploadPixabayVideo', $e);
            //Log::error("uploadPixabayVideo : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'upload video.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
    ...
    public function getCurrentKey()
    {
        try {
            ...
            $response = Response::json(['code' => 200, 'message' => 'Current key fetched successfully.', 'cause' => '', 'data' => $result]);
            $response->headers->set('Cache-Control', Config::get('constant.RESPONSE_HEADER_CACHE'));

        } catch (Exception $e) {
            (new ImageController())->logs('getCurrentKey', $e);
            //      Log::error("getCurrentKey : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'get current key.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
    ...
    public function increaseCurrentKey($currentKey)
    {
        try {
            ...
            $response = Response::json(['code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR').'change API key of pixabay.', 'cause' => $e->getMessage(), 'data' => json_decode('{}')]);
        }
