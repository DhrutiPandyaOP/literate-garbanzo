...
class RegisterController extends Controller
{
    ...
    public function registerUserDeviceByDeviceUdid(Request $request_body)
    {
        try {
            ...
            if (($response = (new VerificationController())->validateRequiredParameter(array('device_udid'), $request)) != '')
                return $response;
...
//          Log::error("registerUserDeviceByDeviceUdid : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function userSignUp(Request $request)
    {
      try {
        ...
        if (($response = (new VerificationController())->validateRequiredParameter(array('first_name', 'email_id', 'password', 'signup_type', 'device_info'), $request)) != '')
          return $response;
        ...
        if (trim($first_name) == "") {
          return Response::json(array('code' => 201, 'message' => 'Please enter valid first name.', 'cause' => '', 'data' => ''));
        }
        ...
        $response = Response::json(array('code' => 200, 'message' => 'Verification link has been sent to your email. Please verify your account by verification link.', 'cause' => '', 'data' => ['user_registration_temp_id' => $uuid]));

      } catch (Exception $e) {
        (new ImageController())->logs("userSignUp", $e);
        $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'register user.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
        DB::rollBack();
      }
      return $response;
    }
    ...
    public function resendVerificationLink(Request $request)
    {
        try {
            ...
            $response = (new VerificationController())->validateRequiredParameter(array('user_registration_temp_id'), $request);
            if ($response != '')
                return $response;
            ...
            if(count($user_id) <=0){
             return Response::json(array('code' => 201, 'message' => 'Invalid registration id.', 'cause' => '', 'data' => json_decode("{}")));
            }
            ...
            if (count($result) > 0) {
                ...
                if (($response = (new VerificationController())->checkIfEmailExist($email_id)) != '')
                    return Response::json(array('code' => 201, 'message' => 'Your email already verified. Please login.', 'cause' => '', 'data' => json_decode("{}")));
                    //return $response;
                ...
                if (strtotime(date('Y-m-d H:i:s')) > strtotime($otp_token_expire)) {
                    ...
                    $response = Response::json(array('code' => 200, 'message' => 'Verification link has been sent to your email. Please verify your account by verification link.', 'cause' => '', 'data' => json_decode("{}")));

                } else {
                    $verification_token = $result[0]->otp_token;
                    $response = Response::json(array('code' => 200, 'message' => 'Your verification link already sent to your email address. Please check your email.', 'cause' => '', 'data' => json_decode("{}")));
...
//            Log::error("resendVerificationLink : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            ...
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'resend verification link.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
            DB::rollBack();
        }
        return $response;
    }
    ...
    public function verifyUserBackUp(Request $request)
    {
        try {
            ...
            $response = (new VerificationController())->validateRequiredParameter(array('user_registration_temp_id', 'token'), $request);
            if ($response != '')
                return $response;
            ...
            if (sizeof($active_user) == 1) {
                ...
                if ($response != '') {
                    return $response = Response::json(array('code' => 200, 'message' => 'Your account is already verified. Please login', 'cause' => '', 'data' => json_decode("{}")));
                }
              ...
              if ($uuid == "") {
                ...
                if (sizeof($select_user_master) == 0) {

                    DB::begintransaction();
                    DB::update('UPDATE user_master SET is_verify = 1 WHERE id = ?', [$user_id]);
                    DB::commit();

                    $response = Response::json(array('code' => 200, 'message' => 'Your account verified successfully. Please login', 'cause' => '', 'data' => json_decode("{}")));
                ...
                return $response = Response::json(array('code' => 201, 'message' => 'Invalid registration id.', 'cause' => '', 'data' => json_decode("{}")));
            }
...
//            Log::error("verifyUser : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function verifyUser(Request $request)
    {
      try {
        ...
        $request = json_decode($request->getContent());
        if (($response = (new VerificationController())->validateRequiredParameter(array('user_registration_temp_id', 'token', 'is_mobile', 'device_info'), $request)) != '') {
        ...
        if ($is_mobile) {
          ...
          return Response::json(array('code' => 200, 'message' => 'Your account verified successfully. Please login', 'cause' => '', 'data' => json_decode("{}")));
        }
        ...
        if ($e instanceof QueryException) {
          ...
          if ($errorCode == 1062) {
            ...
            return Response::json(array('code' => 201, 'message' => config('constant.EXCEPTION_ERROR') . 'verify user, Please try again later', 'cause' => '', 'data' => json_decode("{}")));
          }
        }
        (new ImageController())->logs("verifyUser", $e);
        $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'verify user.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    ...
    public function updateUserProfile(Request $request_body)
    {
        try {
            ...
            $user_profile = (new LoginController())->getUserInfoByUserId($user_id);
            $response = Response::json(array('code' => 200, 'message' => 'Profile updated successfully.', 'cause' => '', 'data' => $user_profile));
...
//            Log::error("updateUserProfile : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
    ...
    public function getUserProfile()
    {
        try {
            ...
            $response = Response::json(array('code' => 200, 'message' => 'Profile fetched successfully.', 'cause' => '', 'data' => $user_detail));
