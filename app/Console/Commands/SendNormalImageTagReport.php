  ...
  public function handle()
  {
    try {
      ...
      if (count($tags) > 0) {
        ...
        $data = array('data' => array("app_name" => $app_name, 'start_date' => $start_date, 'end_date' => $end_date, 'tags' => $tags));

        Mail::send("normal_image_tag_report", $data, function ($message) use ($data) {
...
//      Log::error("SendNormalImageTagReport command handle() : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
      return Response::json(array('code' => 201, 'message' => Config::get('constants.EXCEPTION_ERROR') . ' send normal image search tag report', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
    }
  }
}
