    ...
    try{
      ...
      if(count($users) > 0) {
        ...
        foreach ($users_detail AS $i => $user_detail){
          $users_data[$i] = array($user_detail->user_id,$user_detail->first_name,$user_detail->email_id,$user_detail->my_design_count,$user_detail->my_video_design_count,$user_detail->uploaded_img_count,$user_detail->last_login_time);
        }
        $heading = array("User Id", "First Name", "Email Id","Design Count", "Video Design Count","Upload Image Count", "Last Login Time");
      ...
      $data = array('template'=>$template, 'subject'=>$subject, 'message_body'=>$total_report, 'original_zip_path'=>$original_zip_path, 'user_id_lists'=>$user_id_lists);
      ...
      Mail::send($data['template'], $data, function ($message) use ($data) {
        ...
        if(count($data['user_id_lists']) > 0) {
          $message->attach($data['original_zip_path'], array('as' => 'monthly_report.zip', 'mime' => 'application/pdf'));
        }
      });
