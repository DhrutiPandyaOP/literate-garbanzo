...
//             Log::info('__construct()');
      ...
      $this->logo_json = isset($json_data->logo_json_list) ? $json_data->logo_json_list : array();
      $this->audio_json = isset($json_data->audio_json) ? $json_data->audio_json : array();
...
//      }
      ...
      } else if ($this->video_height > $this->video_width) {
...
//      Log::info('Intro cmd end : ');
      ...
      if (file_exists($output_file) && $result == 0) {
        ...
        $fail_reason = json_encode(array('download_id' => $this->download_id, 'output' => $output, 'result' => $result));
         DB::beginTransaction();
         DB::update('UPDATE video_template_jobs SET status=? WHERE download_id =? ', [$failed, $this->download_id]);
         DB::commit();
        (new userController())->addVideoGenerateHistory($fail_reason,$this->user_id,$this->get_download_id,NULL,$this->content_id,3,$failed);
